...
unit (ones) counts 0→9 while the other three digits hold at 0
tens then counts 0→9 while unit holds at 0
hun (hundreds) counts 0→9
thou (thousands) counts 0→9
When thou reaches 9, the design sets word_active <= '1' and stops counting
...
# 7-segment display - digit select (anodes)
...
This is the general Basys3 board pinout, not a copy of this project's own constraints file — double-check it against Digilent's official Basys3 Master XDC (or your existing .xdc) before building.

Getting Started
Clone this repository
In Vivado, create an RTL project targeting part xc7a35tcpg236-1
Add counter_seg.vhd as a design source and set counter_seg as the top module
Add an .xdc constraints file (see Pin Constraints) as a constraints source
Run Synthesis → Implementation → Generate Bitstream
Program the Basys3 board; press btnC to reset and restart the sequence
Possible Extensions
